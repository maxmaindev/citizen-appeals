...
interface PhotoUploadProps {
  ...
  maxSizeMB?: number
  disabled?: boolean
  showUploadButton?: boolean // Чи показувати кнопку завантаження
}
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {selectedFiles.length > 0 && showUploadButton && (
            <button
              type="button"
            ...
            >
              {isUploading ? 'Завантаження...' : `Завантажити ${selectedFiles.length} фото`}
            </button>
          )}
        </div>
        ...
        {selectedFiles.length === 0 && (
          <div className="mt-4 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
            ...
            <p className="text-sm text-gray-500">Натисніть "Вибрати фото" для завантаження</p>
          </div>
        )}
      </div>
    </div>
